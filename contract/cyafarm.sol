...
pragma solidity >=0.7.0 <0.9.0;
 ...
 contract cyafarm {
    ...
    constructor(address _cutbank,address _cya) { 
...
function seeding() public {
    ...
    emit farmnum(winnum);
    require(mypay[msg.sender] >= price, "not enough point");
...
function choice(uint8 winnum) public {
 ...
 function charge(uint256 _mypay)public  {   //myport에서 가져온 portid
   ...
   require(pay <=  cya.balanceOf(msg.sender),"no cya");
  ...
  function withdraw( )public  {   //수수료를 제외한 금액 이체
   ...
   cutbank.depoup(mymento,mentopay(msg.sender)); //수수료 멘토에게 20% 수당
   mypay[msg.sender] = 0;
   myfee[msg.sender] = 0;
  }
